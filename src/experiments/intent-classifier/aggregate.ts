// src/experiments/intent-classifier/aggregate.ts
// Pure aggregation over repeated runs. No I/O. `det` holds intentAccuracy stats (the primary,
// deterministic score); `payload` holds payloadAccuracy stats (secondary / tiebreaker).
 ...
 * judge-mean desc (only when judge ran) -> PASS-rate desc -> intent-accuracy (det) mean desc ->
 * payload-mean desc -> latency-mean asc (cheaper/faster wins the final tie).
 * Models without a judge/det/payload mean sort last on that key. Pure; returns a new array.
 */
export function rankAggregates(aggs: ModelAggregate[], judgeEnabled: boolean): ModelAggregate[] {
  ...
  const p = (a: ModelAggregate): number => a.payload?.mean ?? -1;
  return [...aggs].sort((a, b) => {
    ...
    const dpl = p(b) - p(a);
    if (dpl !== 0) return dpl;
    return a.latency.mean - b.latency.mean; // lower latency first
  });
}
