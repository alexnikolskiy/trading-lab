# Strategy source: `long_oi` (Long OI)

> **Назначение fixture.** Это human-readable исходное описание стратегии `long_oi` из репозитория
> `trading-platform`. Текст предназначен для подачи в `trading-lab` `strategy.onboard` как
> `StrategyAnalystInput.content` — на его основе StrategyAnalyst должен построить `StrategyProfile` JSON.
>
> **Статус.** Research fixture (read-only анализ). `trading-platform` не изменялся, production-код
