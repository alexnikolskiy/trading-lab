# Исследование границ и поставки SDK экосистемы trading

**Статус:** архитектурные выводы и входные данные для будущего brainstorming  
**Дата:** 2026-06-19  
**Область:** `trading-platform`, `trading-backtester`, `trading-lab`,
`trading-mock-platform`

Этот документ не является feature spec в терминах Superpowers. Перед
реализацией агент должен использовать его как исследовательский контекст,
провести отдельный `superpowers:brainstorming`, проверить актуальное состояние
всех репозиториев и только затем сформировать утверждаемую spec и implementation
plan.
