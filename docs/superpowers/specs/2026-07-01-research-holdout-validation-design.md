...
- **Replace, not flag-gate.** The **initial new-strategy validation** path routes through the Train/Holdout flow. The single-backtest *primitives* are unchanged: the handler-level `runPlatformBacktest` and `finalizeBacktestCompletion` keep serving the hypotheses / retry / Cycle-2 single-backtest path untouched. The experiment flow drives its sanity/train/holdout runs via an injected **`ExperimentRunExecutor`** that does `submitOverlayRun` → `createSubmitted` → `pollOverlayRun` → mark, persisting `BacktestRun` rows itself — so it never calls the void-returning handler-level `runPlatformBacktest` and there is no API mismatch (see §5.0).
...
new strategy (analyst → build, bundle_hash fixed)
  → ExperimentService.runNewStrategyValidation(...)
      ...
      ├─ getRunTrades(sanity.platformRunId) → TradeRecord[]  (entryTs/exitTs; backtester run-scoped)
...
The handler-level `runPlatformBacktest` returns `void` and mints the `backtestRunId` internally — unusable for a flow that needs the `runId` (to fetch trades) and the per-run summary. So `ExperimentService` does **not** call it, nor does it touch the platform/SDK directly. Each run is delegated to an injected **`ExperimentRunExecutor`** (port). The production `BacktesterExperimentRunExecutor` mirrors the handler primitive's order so the resume-safe invariant holds: `submitOverlayRun` → **`backtests.createSubmitted`** (persist immediately, status `submitted`, with the platform run id) → `pollOverlayRun` → `markCompleted` / `markRejected` / `markFailed`, then `mapPlatformComparison` on the completed summary. It returns `{ status (completed|pending|rejected), runId (lab BacktestRun id), platformRunId, comparison?, totalTrades? }`. `ExperimentService` records the `ExperimentRunMember` (with `backtestRunId` = the lab run id, `tradeCount`). The executor needs the bundle + baselineRef + identity (`hypothesisId`/`buildId` required), so they ride on its request. It does **not** create a per-run `Evaluation` or call `markEvaluated` — the single-run `Evaluation` belongs to the old flow; the experiment verdict is the separate `experiment_evaluation`. The handler-level `runPlatformBacktest` is left exactly as-is for the single-backtest path.
...
4. `getRunTrades(sanity.platformRunId)` → `TradeRecord[]` (the trades artifact is scoped to the **backtester** run id, not the lab `BacktestRun` id).
